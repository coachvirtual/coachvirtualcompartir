...
const BOT_DATA = {
    ...
    "inicio": {
        text: "¡Hola! 👋 Soy tu Coach Virtual. Para darte la mejor experiencia, ¿cuál es tu perfil?",
        options: [
    ...
    "est_conocimientos": {
        text: "**Compartir Conocimientos (Estudiante / Padre)**\n\nAprende a navegar por los materiales y enviar trabajos:",
        options: [
    ...
    // ✅ RESPUESTAS INGLÉS
    "richmond_ingreso": {
        text: "**Ingresar a Richmond Studio**\n\nHay dos formas de acceder:\n\n• **Desde Santillana Connect:** Ingresa a tu cuenta y busca el acceso a Richmond Studio.\n• **Desde EDI Santillana:** Ingresa a edi.santillanacompartir.com.co y localiza el acceso.\n\n▶️ Via Connect: <a href=\"#\" onclick=\"openVideoModal('P9ohgrwD_zs')\" class=\"text-emerald-400 font-bold underline\">Ver Video</a> | Via EDI: <a href=\"#\" onclick=\"openVideoModal('wyxu3QD9WX8')\" class=\"text-emerald-400 font-bold underline\">Ver Video</a>",
        options: [{ label: "◀ Volver", topic: "ingles_docente" }]
    },
    "richmond_doc_tareas": {
        text: "**Asignar Tareas en Richmond Studio (Docente)**\n\nDesde tu panel de docente puedes crear y asignar actividades a tus grupos de estudiantes directamente en la plataforma de inglés.\n\n▶️ <a href=\"#\" onclick=\"openVideoModal('ABBwpKFDN8M')\" class=\"text-emerald-400 font-bold underline\">Ver Video Tutorial</a>",
        options: [{ label: "◀ Volver", topic: "ingles_docente" }]
    },
    "richmond_doc_calificar": {
        text: "**Calificar Audios - Pending Marks (Docente)**\n\nRevisar y calificar los audios grabados por tus estudiantes en Richmond Studio. Los encontrarás en la sección de Pending Marks.\n\n▶️ <a href=\"#\" onclick=\"openVideoModal('zC5L4zTrZJA')\" class=\"text-emerald-400 font-bold underline\">Ver Video Tutorial</a>",
        options: [{ label: "◀ Volver", topic: "ingles_docente" }]
    },
    "richmond_doc_isolution": {
        text: "**Acceder al i-solution (Docente)**\n\nEl i-solution es el libro de soluciones del docente. Puedes acceder tanto en modo online como offline según tu conexión.\n\n▶️ <a href=\"#\" onclick=\"openVideoModal('oWvbBcSyjkA')\" class=\"text-emerald-400 font-bold underline\">Ver Video Tutorial</a>",
        options: [{ label: "◀ Volver", topic: "ingles_docente" }]
    },
    "richmond_cookies": {
        text: "**Limpiar Cookies y Caché (Docentes y Estudiantes)**\n\nSi tienes problemas al cargar Richmond Studio, limpiar las cookies y el caché del navegador suele resolver la mayoría de inconvenientes.\n\n▶️ <a href=\"#\" onclick=\"openVideoModal('sKtj-gpv78E')\" class=\"text-emerald-400 font-bold underline\">Ver Video Tutorial</a>",
        options: [
    ...
    "richmond_est_assignments": {
        text: "**¿Dónde ver mis Asignaciones? (Assignments)**\n\nDesde tu panel de estudiante en Richmond Studio puedes revisar todas las actividades asignadas por tu docente, tanto pendientes como entregadas.\n\n▶️ <a href=\"#\" onclick=\"openVideoModal('vSR2f5hqw84')\" class=\"text-emerald-400 font-bold underline\">Ver Video Tutorial</a>",
        options: [{ label: "◀ Volver", topic: "ingles_estudiante" }]
    },
    "richmond_est_markbook": {
        text: "**¿Dónde ver mis Notas? (Markbook)**\n\nEl Markbook es donde encuentras todas tus calificaciones en Richmond Studio. Puedes consultarlo en cualquier momento desde tu perfil.\n\n▶️ <a href=\"#\" onclick=\"openVideoModal('L1sNm4j3ICU')\" class=\"text-emerald-400 font-bold underline\">Ver Video Tutorial</a>",
        options: [{ label: "◀ Volver", topic: "ingles_estudiante" }]
    },
    "richmond_est_mylinks": {
        text: "**My Links - Accesos Directos (Estudiante)**\n\nMy Links te permite guardar y organizar accesos rápidos a los recursos que más usas dentro de Richmond Studio.\n\n▶️ <a href=\"#\" onclick=\"openVideoModal('NG5Jkq0uFeE')\" class=\"text-emerald-400 font-bold underline\">Ver Video Tutorial</a>",
        options: [{ label: "◀ Volver", topic: "ingles_estudiante" }]
    },
    ...
    // 6. CATEGORÍAS DE QUIZZES (Solo Docentes)
    "quiz_categorias": {
...
/* ============================================================
   ÍNDICE DE BÚSQUEDA DE TEXTO LIBRE
   ============================================================ */

// Mapea palabras clave a topics del BOT_DATA
const KEYWORD_INDEX = {
    // Inglés / Richmond
    'richmond': 'richmond_ingreso',
    'inglés': 'ingles_docente',
    'ingles': 'ingles_docente',
    'cookies': 'richmond_cookies',
    'caché': 'richmond_cookies',
    'cache': 'richmond_cookies',
    'markbook': 'richmond_est_markbook',
    'notas': 'richmond_est_markbook',
    'calificaciones': 'pleno_doc_calificar',
    'assignments': 'richmond_est_assignments',
    'asignaciones': 'richmond_est_assignments',
    'pending': 'richmond_doc_calificar',
    'audios': 'richmond_doc_calificar',
    'isolution': 'richmond_doc_isolution',
    'my links': 'richmond_est_mylinks',
    // Conocimientos Docente
    'tarea': 'docente_tarea',
    'tareas': 'docente_tarea',
    'asignar': 'docente_asignar',
    'panel': 'docente_panel',
    'material': 'docente_material',
    'primeros pasos': 'docente_inicio',
    // Quizzes
    'quiz': 'quiz_base',
    'sopa': 'q_juegos',
    'letras': 'q_juegos',
    'crucigrama': 'q_juegos',
    'rúbrica': 'pleno_doc_calificar',
    'rubrica': 'pleno_doc_calificar',
    'verdadero': 'q_vf',
    'falso': 'q_vf',
    'emparejar': 'q_emparejar',
    'clasificar': 'q_clasificar',
    'arrastrar': 'q_arrastrar',
    'matemática': 'q_mates',
    'matematica': 'q_mates',
    'fórmulas': 'q_mates',
    'formulas': 'q_mates',
    // Pleno
    'pleno': 'progreso_docente',
    'evaluación': 'pleno_doc_crear',
    'evaluacion': 'pleno_doc_crear',
    'habilitar': 'pleno_doc_reiniciar',
    'reiniciar': 'pleno_doc_reiniciar',
    // Estudiantes
    'libro': 'est_libroweb',
    'biblioteca': 'est_biblioteca',
    'responder': 'est_tareas',
};

function searchByKeyword(query) {
    const q = query.toLowerCase().trim();
    for (const [keyword, topic] of Object.entries(KEYWORD_INDEX)) {
        if (q.includes(keyword)) return topic;
    }
    return null;
}
...
function renderBotResponse(topicId) {
    ...
    if (data.options && data.options.length > 0) {
        optionsHTML = `<div class="flex flex-col gap-2 mt-4 border-t border-white/10 pt-3" role="group" aria-label="Opciones disponibles">`;
        data.options.forEach(opt => {
            optionsHTML += `
                <button onclick="handleUserClick('${opt.label}', '${opt.topic}')" 
                    class="bg-slate-800/80 hover:bg-indigo-600 border border-indigo-500/30 py-2.5 px-4 rounded-xl text-[11px] font-bold transition-colors text-left flex justify-between items-center group shadow-[0_0_10px_rgba(79,70,229,0.1)] cyber-hover"
                    aria-label="${opt.label.replace(/◀ /, '')}">
                    ${opt.label} <i data-lucide="chevron-right" class="w-4 h-4 group-hover:translate-x-1 transition-transform" aria-hidden="true"></i>
                </button>
            `;
        });
        optionsHTML += `</div>`;
    ...
    botMsg.setAttribute('aria-label', 'Respuesta del Coach Bot');
...
function handleUserClick(label, targetTopic) {
    ...
    userMsg.setAttribute('aria-label', 'Tu mensaje');
    userMsg.innerText = label.replace(/◀ Volver.*/, 'Atrás').replace(/◀ Cambiar Perfil.*/, 'Atrás');
...
function sendFreeText() {
    ...
    showTypingAndRespond(topic || 'no_encontrado', text);
}

// Respuesta cuando no se encuentra nada
BOT_DATA['no_encontrado'] = {
    text: "Hmm, no encontré un tutorial exacto para eso 🤔\n\nPuedes intentar con palabras como: **quiz**, **tarea**, **pleno**, **richmond**, **sopa de letras**, **rúbrica**, o usar el buscador principal.\n\n¿Quieres que te lleve al menú de inicio?",
    options: [
        { label: "Ir al menú principal", topic: "inicio" },
        { label: "Soy Docente 🍎", topic: "menu_profesor" },
...
function showTypingAndRespond(targetTopic) {
    ...
    typingIndicator.innerHTML = '<span class="w-1.5 h-1.5 bg-slate-400 rounded-full animate-bounce" aria-hidden="true"></span><span class="w-1.5 h-1.5 bg-slate-400 rounded-full animate-bounce" style="animation-delay: 0.2s" aria-hidden="true"></span><span class="w-1.5 h-1.5 bg-slate-400 rounded-full animate-bounce" style="animation-delay: 0.4s" aria-hidden="true"></span>';
